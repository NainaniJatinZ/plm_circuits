...
def analyze_layer_pair(
    ...
    clean_layer_caches: TensorDict,
    corr_layer_caches: TensorDict,          # <-- NEW
    clean_layer_errors: TensorDict, 
    device: torch.device, 
    k: int = 50
    ...
    # Δaᵘ  (actual change clean→patch)
    delta_up: torch.Tensor = (
        corr_layer_caches[up_layer] - clean_layer_caches[up_layer]
    ).to(device)
    ...
    for d_idx in down_feats:
        ...
        # multiply by Δaᵘ  →  gᵈ · J_{d,u} · Δaᵘ
        grad_tensor.mul_(delta_up)
...
def main() -> None:
    ...
    parser.add_argument('--output-dir', default='./edge_attribution_results_all_layer_jvp_2B61A', 
    ...
    for i, (up_layer, down_layer) in enumerate(layer_pairs):
        ...
        try:
            result = analyze_layer_pair(
                ...
                clean_layer_caches, corr_layer_caches,  # <-- new argument
                clean_layer_errors, device, k=args.top_k
            )
